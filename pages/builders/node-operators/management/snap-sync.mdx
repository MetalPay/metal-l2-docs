...
*   **Option 1:** Snap sync, no required datadir on Metal L2. This is the recommended option because `op-node` tells `op-geth` to snap sync and then `op-geth` downloads the state at tip and once complete switches to inserting blocks one by one.
...
*   **Option 2:** Archive sync with required [datadir](snapshots) for Metal L2, but no required datadir for other OP Stack networks.
...
For Metal L2, the [bedrock datadir](snapshots) is required. For other OP Stack networks, no datadir is required. 
