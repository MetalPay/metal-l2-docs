...
Specifically, Optimistic Rollups leverage the consensus mechanism (like PoW or PoS) of their parent chain instead of providing their own. In Metal L2's case, this parent blockchain is Ethereum.
...
This is the way that Metal L2 inherits the availability and integrity guarantees of Ethereum.
...
This is important because writing to L1 is [the major cost of Metal L2 transactions](/stack/transactions/fees).
...
In Metal L2, blocks are produced every two seconds, regardless of whether they are empty (no transactions), filled up to the block gas limit with transactions, or anything in between.
...
2.  Transactions submitted on L1 (called *deposits*) are included in the chain in the appropriate L2 block.
    ...
    This provides Metal L2 with L1 Ethereum level censorship resistance.
...
For the moment, [The Optimism Foundation](https://www.optimism.io/) runs the only block producer on Metal L2. 
...
Optimism is designed so that users can send arbitrary messages between smart contracts on L2 (Metal L2, Metal L2 Testnet, etc.) and the underlying L1 (Ethereum mainnet, Sepolia, etc.).
...
Metal L2 uses this functionality in the Standard bridge to allow users to deposit tokens from Ethereum to Metal L2 and also to allow withdrawals of the same tokens from Metal L2 back to Ethereum.
...
### Moving from Ethereum to Metal L2

In Optimism terminology, transactions going from Ethereum (L1) to Metal L2 (L2) are called *deposits*.
...
### Moving from Metal L2 to Ethereum

Withdrawals (the term is used for any Metal L2 to Ethereum message) have three stages:
...
2.  Wait for the next output root to be submitted to L1 (you can see this on [the SDK](https://sdk.optimism.io)) and then submit the withdrawal proof using `proveWithdrawalTransaction`.
    ...
    This protects Metal L2 users against a whole class of potential bridge vulnerabilities.
...
In an Optimistic Rollup, state commitments are published to L1 (Ethereum in the case of Metal L2) without any direct proof of the validity of these commitments.
...
Once a commitment is considered final, smart contracts on Ethereum can safely accept withdrawal proofs about the state of Metal L2 based on that commitment.
...
It's important to note that a successful challenge does not roll back Metal L2 itself, only the published commitments about the state of the chain.
The ordering of transactions and the state of Metal L2 is unchanged by a fault proof challenge.
